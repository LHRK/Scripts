...
proc orderparam-c2 { result { seltext all } } {
  ...
  for { set i 2 } { $i <= 20 } { incr i } {
    ...
    for { set frame 0 } { $frame < $n } { incr frame } {
      ...
      #$hz frame $frame
      #$h9 frame $frame
      #If I update hz and h9 every frame, I get the same profile for
      #both tcl and python script
      set cpx [$cp get x]
      set cpy [$cp get y]
      set cpz [$cp get z]
      ...
      if { [$h9 num] != 0 } {
        set hxx [vecsub $cpx [$h9 get x]]
        set hxy [vecsub $cpy [$h9 get y]]
        set hxz [vecsub $cpz [$h9 get z]]
        foreach dx $hxx dy $hxy dz $hxz {
          set norm2 [expr {$dx*$dx + $dy*$dy + $dz*$dz}]
          set sum [expr {$sum + $dz*$dz/$norm2}]
        }
